...
uint8_t BroadCastIP[4] = {192,168,0,101};   // Replace with  Your Destination  IP Address
uint16_t DestinationPort = 6000;  

uint8_t alert_msg[] = "hello amal ";
...
uint8_t IP[4]          = {192,168,0,110};			// IP Address
uint8_t Gateway[4]     = {192,168,0,1};			// Gateway Address
uint8_t Subnet [4]     = {255,255,255,0};			// Subnet Address
uint8_t MAC[6]         = {0x00,0x08,0xDC,0x01,0x04,0x05};	// MAC Address
...
int main() {
    ...
    */
	...
	printf("SUBR : %d.%d.%d.%d\r\n",SN[0],SN[1],SN[2],SN[3]);
	printf("SIPR : %d.%d.%d.%d\r\n",IP[0],IP[1],IP[2],IP[3]);
    W5300_Bus_Write_Indirect(_RTR_,0x0FA0);   // RTR register
     
    /* Manually Initilizing The Socket 0*/
    W5300_Bus_Write_Indirect(0x200,0x001); // SO_MR1 TCP -0x001
    printf("configured as TCP in case of 0x01 = %d\n",W5300_Bus_Read_Indirect(0x201));
    W5300_Bus_Write_Indirect(0x20A,0x1770); // sets port number to 49153 0xC001
    printf("port number is %d\n",W5300_Bus_Read_Indirect(0x20A));
    W5300_Bus_Write_Indirect(0x202,0x001); // sets open Command
    printf(" open command  is 0x01 , socket status expecting zero  %d\n",W5300_Bus_Read_Indirect(0x202));
    printf("0x13 SOCK_INIT - means socket is open as TCP Mode %d\n",W5300_Bus_Read_Indirect(0x208)&0xFF);  // Socket initilized 
        
    socket(0,Sn_MR_TCP,DestinationPort,SF_TCP_ALIGN ); /* Socket(Socket 0 used) creation - used IO Library Code*/
    sleep_ms(100);
    connect(0,BroadCastIP,DestinationPort);
    sleep_ms(100);
    while((W5300_Bus_Read_Indirect(Sn_SSR(0))&0xFF) != SOCK_ESTABLISHED){
        printf("socket not initilized ");
    }
    sleep_ms(5000);
    disconnect(0);   /* Socket Disconnects */
}
